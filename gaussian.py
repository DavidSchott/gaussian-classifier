...
def MyMean(feats):
    ...
    for i in range(len(feats[0])):
        for j in range(len(feats)):
            sum_cols = sum_cols + feats[j][i]
        mean_cols.append(sum_cols)
        sum_cols = 0
    for entry in range(len(mean_cols)):
        mean_cols[entry] = mean_cols[entry] / (len(feats))
...
def sharedCovGaussAcc(f,foldNo):
    tupledict = sharedGaussianModel(f,foldNo)
    j = 1
    for i in range(len(f)):
        real_class = knn.data["fold"+str(foldNo)+"_classes"][i]
        if (tupledict[str(i+1)][1] == real_class[0]):
